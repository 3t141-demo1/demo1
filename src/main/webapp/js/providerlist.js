...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:path+"/jsp/deleteByProvider.action",
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/jsp/queryByIdProvider.action?method=view&provider.id="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/jsp/queryByIDUpdateProvider.action?method=modify&provider.id="+ obj.attr("proid");
	});
